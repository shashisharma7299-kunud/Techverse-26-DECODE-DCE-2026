...
const els = {
  ...
  resetAllTimers: document.querySelector("#resetAllTimers"),
...
els.settingsForm.addEventListener("submit", async (event) => {
  ...
  try {
    adminData = await adminApi("/api/admin/settings", {
      ...
      resetAllTimers: els.resetAllTimers.checked
    });
    renderAdmin(adminData);
    alert(els.resetAllTimers.checked ? "Time applied to all participants." : "Default event time saved.");
  } catch (error) {
    alert(error.message);
  }
});

els.questionEditor.addEventListener("input", () => {
  questionEditorDirty = true;
});

els.questionEditor.addEventListener("focusin", () => {
  questionEditorDirty = true;
});
...
function renderAdmin(data) {
  ...
  if (!questionEditorDirty) renderQuestionEditor(data.questions);
...
async function savePlayerTime(rowPlayerId) {
  ...
  try {
    ...
    renderAdmin(adminData);
    alert("Participant time updated.");
  } catch (error) {
    alert(error.message);
  }
}
...
function addQuestionEditor() {
  if (!adminData) return;
  adminData.questions = readQuestionEditor();
  adminData.questions.push({ text: "", answer: "" });
  questionEditorDirty = true;
  renderQuestionEditor(adminData.questions);
  const lastQuestion = els.questionEditor.querySelector(`[data-question-text="${adminData.questions.length - 1}"]`);
  lastQuestion?.focus();
}
...
function readQuestionEditor() {
  return [...els.questionEditor.querySelectorAll(".question-edit")].map((row, index) => ({
    text: row.querySelector(`[data-question-text="${index}"]`)?.value.trim() || "",
    answer: row.querySelector(`[data-question-answer="${index}"]`)?.value.trim() || ""
  }));
}
